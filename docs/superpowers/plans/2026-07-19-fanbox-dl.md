...
- **conflictAction は `uniquify` 固定**(spec §8)。`overwrite` はコード・型・UI のどこにも存在させない。
...
- Produces(以降の全タスクが import する):
  ...
  - `FileItem { contentType, url, filename, ext, seq, total, idemKey, stableContentId, refetch: {postId, stableContentId, index} }`
  - `ContentBlock { blockOrdinal: number, contentType, files: FileItem[] }`
  - `PostData { postId, postTitle, creator, creatorId, fee: number, publishedAt: Date, updatedAtIso: string, restricted: boolean, postType: string, contents: ContentBlock[] }`
...
**グルーピング規則の明確化**(spec §6 の「連続する同種」の解釈。plan で確定): article の blocks 走査では **メディアブロック(image/file)以外(p 等)はグループを切らない**。グループはメディア種が変わったときだけ切り替わる(image 群→file 群)。典型 article は image と p が交互に並ぶため、p で切るとギャラリーが全部単発になり zip が永久に成立しないため。
...
```ts
import { describe, it, expect } from "vitest";
import { parsePost } from "../src/fanbox/parse";
...
interface RawFile { id: string; name: string; extension: string; url: string }
...
export function parsePost(json: any): PostData {
  ...
  if (post.type === "image") {
    ...
    for (const blk of b.blocks ?? []) {
      ...
      // p / embed / url_embed / 未知ブロックは無視(グループも切らない)
    }
  }
  ...
  for (const g of groups) {
    ...
    const files: FileItem[] = g.items.map((it, i) => ({
      ...
      refetch: { postId, stableContentId: it.stableContentId, index: i },
    }));
...
describe("classifyDownloadError (spec §6)", () => {
  ...
  it("SERVER_* は needs_page(URL 失効・編集の可能性)", () => {
    expect(classifyDownloadError("SERVER_FORBIDDEN")).toBe("needs_page");
...
export function applyNeedsPageRecovery(l: Ledger, postId: string, fresh: Array<{stableContentId: string; url: string; basePath: string}>, opts: {now: number; newLeaseToken: () => string; validatePath: (p: string) => string | null}): { ledger: Ledger; toStart: JobRecord[]; missing: string[] };
...
import {
  ...
  applyClearTerminal, applyPruneSweep, findLeasesWithoutDownloadId,
...
describe("lease/CAS ライフサイクル", () => {
  ...
  it("interrupted + needs_page / terminal_error", () => {
    const { l, token } = enq();
    const started = applyDownloadStarted(l, "111:image:a", token, 1);
    expect(applyDownloadInterrupted(started, "111:image:a", token, "needs_page", "SERVER_FORBIDDEN", mkTok, 0).jobs["111:image:a"].state).toBe("needs_page");
...
describe("needs_page 回復 (spec §6 / §14-2)", () => {
  const toNeedsPage = (): Ledger => {
    const { l, token } = enq();
    return applyDownloadInterrupted(applyDownloadStarted(l, "111:image:a", token, 1), "111:image:a", token, "needs_page", "SERVER_FORBIDDEN", mkTok, 0);
  };
  it("stableContentId 一致で新 URL に再バインド(世代交代)", () => {
    const r = applyNeedsPageRecovery(toNeedsPage(), "111",
      ...
      { now: 50_000, newLeaseToken: mkTok, validatePath: () => null });
    expect(r.toStart).toHaveLength(1);
...
export function applyDownloadRequestFailed(l: Ledger, idemKey: string, leaseToken: string, error: string): Ledger {
  return withCas(l, idemKey, leaseToken, (r) => ({ ...r, state: "error", error, leaseToken: undefined }));
}

export function applyDownloadComplete(l: Ledger, idemKey: string, leaseToken: string, actualFilename: string, doneAt: number): Ledger {
  return withCas(l, idemKey, leaseToken, (r) => ({
    ...r,
    state: "done", doneAt, actualFilename,
...
export function applyDownloadInterrupted(
  ...
  return withCas(l, idemKey, leaseToken, (r) => {
    ...
    if (action === "needs_page") {
      return { ...r, state: "needs_page", error, leaseToken: undefined, leasedAt: undefined };
    }
    return { ...r, state: "error", error, leaseToken: undefined, leasedAt: undefined };
  });
}
...
export function applyNeedsPageRecovery(
  ...
  opts: { now: number; newLeaseToken: () => string; validatePath: (p: string) => string | null },
): { ledger: Ledger; toStart: JobRecord[]; missing: string[] } {
  const jobs = { ...l.jobs };
  ...
  for (const rec of Object.values(l.jobs)) {
    ...
    if (!f) {
      jobs[rec.idemKey] = { ...rec, state: "error", error: "投稿が編集され該当ファイルは存在しない" };
      missing.push(rec.idemKey);
      continue;
    }
    const gen = rec.generation + 1;
    const relPath = canonicalRelPath(f.basePath, gen);
    const err = opts.validatePath(relPath);
    if (err) {
      jobs[rec.idemKey] = { ...rec, state: "error", error: `${relPath}: ${err}` };
      missing.push(rec.idemKey);
      continue;
    }
  ...
  return { ledger: { ...l, jobs }, toStart, missing };
}
...
export function applyPruneSweep(
  ...
  for (const rec of Object.values(l.jobs)) {
    if (rec.state === "done" && typeof rec.doneAt === "number" && now - rec.doneAt > maxAgeMs) drop(rec);
  }
  const terminals = Object.values(jobs)
    .filter((r) => TERMINAL_STATES.has(r.state))
    .sort((a, b) => (a.doneAt ?? a.leasedAt ?? 0) - (b.doneAt ?? b.leasedAt ?? 0));
...
**Files:**
- Create: `src/background/fanbox-api.ts`
...
```ts
import { describe, it, expect } from "vitest";
import { fetchPostInfo, validatePostInfo } from "../src/background/fanbox-api";
...
describe("validatePostInfo (spec §4a schema 検証)", () => {
  it("id 一致 + type 既知形状で null", () => {
    expect(validatePostInfo({ body: { post: { id: "5", type: "article" } } }, "5")).toBeNull();
  });
  it("postId 不一致はエラー", () => {
...
`src/background/fanbox-api.ts`:

```ts
// spec §4a canonical: SW(extension context)からの cookie 付き post.info fetch。
...
```ts
export interface DownloadRequestMessage { kind: "download"; postId: string; force: boolean; }
...
**fantia-dl との違い**: parse/render/zip はすべて SW に移ったため(spec §3)、content script は「postId を取って SW に依頼して結果を表示する」だけの薄い層。page script・CSRF・fflate import は存在しない。SPA 遷移(spec §12)は popstate + 1 秒間隔の URL 監視で検知する。
...
```ts
// content script -> SW のメッセージ。parse/render/zip は SW 側(spec §3)なので
// ここは依頼と結果表示のためだけの薄い契約。
...
export function collectZipSources(
  files: Array<{ url: string; idemKey: string }>, postId: string,
  deps: { fetchFn?: (url: string, init?: RequestInit) => Promise<{ ok: boolean; status: number; arrayBuffer(): Promise<ArrayBuffer> }>; budget?: number },
...
export function handleZipDownloadChange(delta: chrome.downloads.DownloadDelta): Promise<boolean>; // zip 由来なら処理して true
...
```ts
import { describe, it, expect } from "vitest";
import { zipEligible, collectZipSources, ZIP_SOURCE_BUDGET_BYTES, ZIP_MAX_FILES } from "../src/background/zip";
...
describe("collectZipSources (spec §7b バジェット)", () => {
  const okFetch = (size: number) => async () => ({ ok: true, status: 200, arrayBuffer: async () => new ArrayBuffer(size) });
  it("直列 fetch して buffers を返す", async () => {
    const r = await collectZipSources(block(2).files, "1", { fetchFn: okFetch(10) });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.buffers.size).toBe(2);
  });
  it("累積バイトが budget を超えたら中止 error", async () => {
    const r = await collectZipSources(block(3).files, "1", { fetchFn: okFetch(60), budget: 100 });
    expect(r.ok).toBe(false);
  ...
  it("件数上限超過は fetch せず即 error", async () => {
    ...
    const r = await collectZipSources(files, "1", { fetchFn: (async () => { called++; return { ok: true, status: 200, arrayBuffer: async () => new ArrayBuffer(1) }; }) });
    expect(r.ok).toBe(false);
  ...
  it("allowlist 違反 URL は fetch せず error (spec §4a: zip ソース fetch も対象)", async () => {
    ...
    const r = await collectZipSources([bad], "1", { fetchFn: okFetch(1) });
    expect(r.ok).toBe(false);
  });
  it("fetch 失敗(403 等)は error", async () => {
    const r = await collectZipSources(block(2).files, "1", { fetchFn: async () => ({ ok: false, status: 403, arrayBuffer: async () => new ArrayBuffer(0) }) });
    expect(r.ok).toBe(false);
  });
...
type BinFetch = (url: string, init?: RequestInit) => Promise<{ ok: boolean; status: number; arrayBuffer(): Promise<ArrayBuffer> }>;
...
export async function collectZipSources(
  ...
  for (const f of files) {
    ...
    let r: Awaited<ReturnType<BinFetch>>;
    try {
      r = await fetchFn(f.url, { credentials: "include" });
    } catch (e) {
    ...
    used += buf.byteLength;
    if (used > budget) return { ok: false, error: `zip ソース合計がバイト上限(${budget})を超過` };
...
**Files:**
- Modify: `src/background/service-worker.ts`(Task 2 の gate 版を全面置き換え)
...
```json
{
  ...
  "host_permissions": ["https://*.fanbox.cc/*"],
  "content_scripts": [
    { "matches": ["https://*.fanbox.cc/posts/*", "https://www.fanbox.cc/@*/posts/*"], "js": ["content/content-script.js"], "run_at": "document_idle" }
  ],
...
import { validateMediaUrl } from "../core/url-allowlist";
import { parsePost } from "../fanbox/parse";
import { fetchPostInfo, validatePostInfo } from "./fanbox-api";
...
// force 前処理(spec §7c-3): 対象投稿の非 terminal ジョブを解決してから世代交代する
async function settleInFlight(postId: string): Promise<string[]> {
  const errors: string[] = [];
  ...
  for (const rec of Object.values(ledger.jobs)) {
    ...
    if (rec.downloadId === undefined && rec.leaseToken) {
      // lease 未解決の窓(spec §7c-3): promise があれば待つ、無ければ adoption 検索
      const p = inFlightDownloads.get(rec.leaseToken);
      if (p) { await p; }
      else {
        const items = await chrome.downloads.search({ url: rec.url });
        const hit = findAdoptable(items.map((d) => ({ id: d.id, url: d.url ?? "", filename: d.filename, startTime: d.startTime ?? "", state: d.state })), { url: rec.url, relPath: rec.relPath, leasedAt: rec.leasedAt ?? 0 });
        if (hit && hit.state === "in_progress") { try { await chrome.downloads.cancel(hit.id); } catch {} }
      }
    }
    const after = (await store.read()).jobs[rec.idemKey];
    if (after?.downloadId !== undefined && (after.state === "pending" || after.state === "requested")) {
      try { await chrome.downloads.cancel(after.downloadId); } catch {}
      // terminal 遷移の有界待機(onChanged が ledger を進める。10 秒でタイムアウト)
      const deadline = Date.now() + 10_000;
      while (Date.now() < deadline) {
        const cur = (await store.read()).jobs[rec.idemKey];
        if (!cur || cur.state === "done" || cur.state === "error" || cur.state === "needs_page") break;
        await new Promise((r) => setTimeout(r, 200));
      }
      const fin = (await store.read()).jobs[rec.idemKey];
      if (fin && (fin.state === "pending" || fin.state === "requested")) {
        errors.push(`${rec.relPath}: 進行中ダウンロードの停止がタイムアウトしました`);
      }
    }
  }
  return errors;
...
async function handleDownloadRequest(msg: DownloadRequestMessage): Promise<DownloadResponse> {
  ...
  const fetched = await fetchPostInfo(msg.postId);
  if (!fetched.ok) { res.errors.push(fetched.error); return res; }
  const schemaErr = validatePostInfo(fetched.json, msg.postId);
  if (schemaErr) { res.errors.push(schemaErr); return res; }

  const post = parsePost(fetched.json);
  if (post.restricted) { res.notices.push("未加入プランの投稿のためダウンロードできません"); return res; }
  if (post.contents.length === 0) { res.notices.push(`この投稿(type: ${post.postType})に DL 対象はありません`); return res; }
  ...
  try {
    const rec = await store.commit((l) => {
      const r = applyNeedsPageRecovery(l, post.postId, fresh, { now: Date.now(), newLeaseToken, validatePath: vp });
  ...
  if (msg.force) {
    const errs = await settleInFlight(post.postId);
  ...
  for (const b of post.contents) {
    let zipDone = false;
    if (zipEligible(b, s)) {
      const collected = await collectZipSources(b.files.map((f) => ({ url: f.url, idemKey: f.idemKey })), post.postId, {});
      if (collected.ok) {
        try {
          ...
          else res.notices.push(`zip にできないため個別ダウンロードに切り替えました: ${dl.error}`);
        } catch (e) {
          res.notices.push(`zip にできないため個別ダウンロードに切り替えました: ${e instanceof TemplateError ? e.message : String(e)}`);
        }
      } else {
        res.notices.push(`zip にできないため個別ダウンロードに切り替えました: ${collected.error}`);
      }
    }
...
chrome.downloads.onChanged.addListener(async (delta) => {
  ...
  const [item] = await chrome.downloads.search({ id: delta.id });
  const action = classifyDownloadError(delta.error?.current ?? item?.error ?? undefined);
  const after = await store.commit((l) => {
    const l2 = applyDownloadInterrupted(l, rec.idemKey, token, action, delta.error?.current ?? "interrupted", newLeaseToken, Date.now());
...
(async () => {
  ...
  for (const rec of findLeasesWithoutDownloadId(ledger)) {
    ...
    if (hit) {
      ...
      void startDownload(rec); // 最悪ケースは uniquify の重複 1 個(spec §7c-1)
    }
  }
  ...
  for (const rec of Object.values(ledger.jobs)) {
    ...
    if (!d) { void startDownload(rec); continue; }
    if (d.state === "complete") {
      ...
      const action = classifyDownloadError(d.error ?? undefined);
      const after = await store.commit((l) => {
        const l2 = applyDownloadInterrupted(l, rec.idemKey, rec.leaseToken!, action, d.error ?? "interrupted", newLeaseToken, Date.now());
        return { ledger: l2, result: l2.jobs[rec.idemKey] };
      });
...
- `options.ts`: conflictAction の読み書き(要素参照・load 時の反映・save 時の収集)を削除。`fantia` という文言・デフォルトテンプレート表示があれば `fanbox` 系(Task 3 の DEFAULT_SETTINGS)に置換
- 撤去後の確認: `grep -rn "conflictAction\|overwrite" src/options public/options` が **0 件**であること

- [ ] **Step 2: ビルドが 4 entry 全部通ることを確認**

```bash
wsl.exe -e bash -lc 'cd /home/shishi/dev/src/github.com/shishi/fanbox-dl && bun run test && bun run typecheck && bun run build && grep -c conflictAction dist/options/options.js; true'
```

Expected: テスト全 PASS・build 成功・grep 0
...
- **zip モード**(既定 ON): 2 枚以上の画像ギャラリーはブロック単位で 1 zip(`images_$contentId.zip`)。zip は**再開不可の一括処理**で履歴に残らない。大きい投稿や確実性が要る場合は通常 DL 推奨(zip にできない場合は自動で個別 DL に切り替わる)。ソース合計 100MB / 100 件が上限
...
4. 同じ投稿をもう一度 ⬇ → 「0 件開始」(dedup)になること
5. **(§13-2・有料)** 有料プランに加入している場合のみ: 有料投稿の DL が成功するか。**403 なら spec §7a の通り明示エラーになることを確認し、フォールバックの別 spec 化を課題として記録**(実装はしない)
